...
function [EEGdata] = pop_subcomp( EEG, components, plotag )
...
if nargin < 1
   ...
   return;
    ...
    %display(components);
 %   display(max(components));
%    display(size(EEG.icaweights,1));
...
compproj = EEG.icawinv(:,component_keep)*EEG.icaact(component_keep,:);
EEGdata=compproj;
%{
