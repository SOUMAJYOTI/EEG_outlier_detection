function signal = peak_epoch (frames, epochs, srate, peakfr, position, noisy_epochs, tjitter, wave)
...
disp(noisy_epochs);
%generating peak
signal = zeros (1, (epochs+1) * frames);
for trial = 2:epochs+1
 pos = position + round(randn(1)*tjitter);
 if ismember(trial, noisy_epochs)==1
    for i=1:frames
        phase = (i-pos)/srate*2*pi*peakfr;
        if wave | (phase < pi/2 & phase > -pi/2)
            signal((trial-1)*frames+i) = cos(phase);%*((round(randn(1)*10)/9)); %signal((trial-1)*frames+i) = cos(phase);
        end
    end
 end
end
